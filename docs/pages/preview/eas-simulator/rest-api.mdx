...
A `web-preview-only` session provides the web preview without a controller. It does not support an idle timeout. Other session types provide a controller and include a web preview on supported iOS sessions. Sessions created before `serve-sim` was removed still return `"type": "serve-sim"`.
...
Open `webPreviewUrl` in a desktop browser to view supported iOS sessions. Do not open it on the remote device. Android sessions do not currently provide a browser preview.
