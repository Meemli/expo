...
export async function serializeBitSetChunksAsync(
  ...
  for (const atom of plan.chunks) {
    ...
    if (entryIndices.length === 1) {
      ...
      const hash = createHash('sha256')
        ...
        .digest('hex')
        .slice(0, 16);
      ownerChunk = new Chunk(`/__shared-${hash}.js`, new Set(), graph, options, true);
  ...
  for (const [entryPath, atoms] of plan.requiredChunksByEntryPath) {
    ...
    if (facade !== entryChunk && [...requiredChunks].every((chunk) => chunk === entryChunk)) {
      ...
      entryChunk.entryPaths.push(entryPath);
      requiredChunksByEntryPath.set(entryPath, []);
      continue;
    }
    for (const ownerChunk of requiredChunks) {
      if (ownerChunk !== entryChunk && ownerChunk !== facade) facade.requiredChunks.add(ownerChunk);
    }
  ...
  for (const chunk of chunks) {
    chunk.getBitSetChunkTargets = (dependency) => {
    ...
    for (const module of this.deps) {
      for (const dep of module.dependencies.values()) {
        if (isResolvedDependency(dep) && dep.data.data.asyncType) {
          if (this.getBitSetChunkTargets) {
            for (const target of this.getBitSetChunkTargets(dep)) targets.add(target);
            continue;
          }
